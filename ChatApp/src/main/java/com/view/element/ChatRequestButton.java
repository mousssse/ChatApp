...
public class ChatRequestButton extends Button {
			...
			public void handle(ActionEvent event) {
				...
				case acceptRequest:
					...
					ListenerManager.getInstance().fireOnMessageToSend(OnlineUsersManager.getInstance().getLocalUser(),
							remoteUser, null, LocalDateTime.now(), MessageType.ACCEPT_REQUEST);
					break;
				case cancelRequest:
					ListenerManager.getInstance().fireOnChatCancelRequest(remoteUser);
					ListenerManager.getInstance().fireOnMessageToSend(OnlineUsersManager.getInstance().getLocalUser(),
							remoteUser, null, LocalDateTime.now(), MessageType.CANCEL_REQUEST);
					break;
				case requestChat:
					ListenerManager.getInstance().fireOnChatRequest(remoteUser);
					ListenerManager.getInstance().fireOnMessageToSend(OnlineUsersManager.getInstance().getLocalUser(),
							remoteUser, null, LocalDateTime.now(), MessageType.REQUEST_CONVERSATION);
					break;
				case endChat:
					ListenerManager.getInstance().fireOnMessageToSend(OnlineUsersManager.getInstance().getLocalUser(),
							remoteUser, null, LocalDateTime.now(), MessageType.END_CONVERSATION);
	...
	/**
	 * Sets the associated remote user to the button and adds the button in the
	 * ChatAppStage map of all buttons
	 * 
